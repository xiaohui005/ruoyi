...
public class YudaoWxClientConfiguration {
    ...
    @Bean
    @ConditionalOnProperty(prefix = WxMpProperties.PREFIX, name = "app-id")
    ...
    public WxMpService wxMpService(WxMpConfigStorage configStorage, WxMpProperties properties) {
        HttpClientType httpClientType = properties.getConfigStorage() != null
                ? properties.getConfigStorage().getHttpClientType() : null;
        WxMpService service;
        ...
        } else if (httpClientType == HttpClientType.HttpComponents) {
            service = new WxMpServiceHttpComponentsImpl();
        } else {
